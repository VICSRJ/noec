...
function NavigationItemView({ item, depth, pathname, renderNavigation }: {
  item: NavigationItem;
  depth: number;
  pathname: string;
        ...
        <Column fillWidth marginTop="2">
          {layout.sidebar.collapsible ? (
            <Accordion
              ...
              paddingTop="4"
              open={open}
              title={<Row textVariant="label-strong-s" onBackground="brand-strong">{item.title}</Row>}
            >
              {renderNavigation(item.children, depth + 1)}
            </Accordion>
          ) : (
            <Column gap="2" paddingLeft="4" paddingTop="4">
              <Row paddingY="12" paddingLeft="8" textVariant="label-strong-s" onBackground="brand-strong">{item.title}</Row>
              {renderNavigation(item.children, depth + 1)}
            </Column>
          )}
        </Column>
  ...
  return (
    <ToggleButton fillWidth horizontal="between" selected={isSelected} className={depth === 0 ? styles.navigation : undefined} href={`${BASE_PATH}/${slug}`}>
...
const ResourceLink = React.memo(({ href, icon, label, pathname }: { href: string; icon: string; label: string; pathname: string }) => {
  const selected = pathname === `${BASE_PATH}${href}` || pathname === href;
  return (
    <ToggleButton fillWidth horizontal="between" selected={selected} className={styles.navigation} href={`${BASE_PATH}${href}`}>
...
function SidebarContent({ navigation, pathname }: { navigation: NavigationItem[]; pathname: string }) {
  const renderNavigation = (items: NavigationItem[], depth = 0): React.ReactNode => items.map((item) => (
    <NavigationItemMemo key={normalizeSlug(item.slug)} item={item} depth={depth} pathname={pathname} renderNavigation={renderNavigation} />
  ));
    ...
    <>
      ...
      {(routes["/roadmap"] || routes["/changelog"]) && (
        <Column gap="2" marginTop="32" paddingLeft="4">
          ...
          {routes["/roadmap"] && <ResourceLink href="/roadmap" icon="roadmap" label="Roadmap" pathname={pathname} />}
          {routes["/changelog"] && <ResourceLink href="/changelog" icon="changelog" label="Changelog" pathname={pathname} />}
        </Column>
      )}
    </>
  );
...
export function Sidebar({ initialNavigation, ...rest }: SidebarProps) {
  const [navigation, setNavigation] = useState<NavigationItem[]>(initialNavigation || globalNavigationCache || []);
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_PATH}/api/navigation`, { signal: controller.signal })
      ...
      .then((data: NavigationItem[]) => {
        const value = Array.isArray(data) ? data : [];
        globalNavigationCache = value;
        setNavigation(value);
      })
      .catch((error: unknown) => {
  ...
  return (
    <Column width={layout.sidebar.width} minWidth={layout.sidebar.width} position="sticky" top="56" fitHeight gap="2" as="nav" overflowY="auto" paddingX="8" paddingY="16" style={containerStyle} {...rest}>
