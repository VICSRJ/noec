...
interface HeaderProps { initialNavigation?: NavigationItem[]; }

export function Header({ initialNavigation = [] }: HeaderProps) {
  const [sidebarVisible, setSidebarVisible] = useState(false);
  const [isMac] = useState(() => typeof navigator !== "undefined" && navigator.userAgent.toLowerCase().includes("mac"));
  const { theme, setTheme } = useTheme();

  const docsItems = useMemo(() => {
    const convert = (items: NavigationItem[]): any[] => items.flatMap((item) => {
      if (item.children?.length) return convert(item.children).map((child) => ({ ...child, section: item.title }));
      const slug = item.slug.replace(/^src[\\/]content[\\/]/, "").replace(/\\/g, "/");
      return [{ id: slug, name: item.label || item.title, section: "Documentation", shortcut: [], keywords: item.keywords || `${item.title.toLowerCase()}, docs, documentation`, href: `${BASE_PATH}/${slug}`, icon: item.navIcon || "document" }];
    });
    return convert(initialNavigation);
  }, [initialNavigation]);

  const kbar = useMemo(() => [
    { id: "home", name: "Home", section: "Navigation", shortcut: [], keywords: "home, landing page", href: `${BASE_PATH}/`, icon: "home" },
    ...
    ...docsItems,
    { id: "theme-toggle", name: theme === "dark" ? "Light mode" : "Dark mode", section: "Theme", shortcut: [], keywords: "light mode, dark mode, theme, toggle", perform: () => setTheme(theme === "dark" ? "light" : "dark"), icon: theme === "dark" ? "light" : "dark" },
    ...
    <Flex as="header" background="page" horizontal="center" position="sticky" zIndex={9} fillWidth vertical="center" paddingY="12" paddingRight="16" paddingLeft="12" borderBottom="neutral-alpha-medium">
      <Row maxWidth={layout.header.width} vertical="center" horizontal="between" gap="l">
        <Row fillWidth vertical="center" gap="8">
          <Animation triggerType="click" active={sidebarVisible} slideDown={1} scale={0.9} blur={0.5} duration={200} trigger={<NavIcon hide m={{ hide: false }} onClick={() => setSidebarVisible((value) => !value)} isActive={sidebarVisible} />}>
